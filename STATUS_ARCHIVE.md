...
Closed/superseded/purely-historical threads moved out of `STATUS.md` on
2026-07-28 to keep that file focused on what's still open (STATUS.md had
grown to 2558 lines, most of it resolved history every fresh session was
otherwise instructed to read first). Same rule as `STATUS.md` applies: update
in place, never create a dated/versioned copy — `git log` is the history.
Read this only when `STATUS.md`'s one-line pointer isn't enough context.

Original chronological order preserved (newest first, matching `STATUS.md`'s
own convention at the time these were archived).
