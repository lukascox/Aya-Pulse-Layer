#!/bin/sh
# pulse_lite v3.5 -- AutoTDP daemon, KONKR Pocket FIT / Snapdragon G3 Gen 3
...
#   machines (CPU_TIER / CPU_HTIER and GPU_TIER / GPU_HTIER). Previously a
#   single shared TIER drove both CPU freq caps and GPU pwrlevel together,
#   which broke down badly in CPU-light/GPU-heavy workloads (Switch emulation:
#   GPU 80-90% busy, CPU ~20% busy -- old logic forced ALL cores to max just
#   because GPU was in heavy) and in GPU-light/CPU-irrelevant workloads
#   (launcher/menu/GBA: brief GPU busy% spikes were escalating CPU clocks via
#   the old MEDIUM_STUCK_TICKS safety valve even at gpu=12-13%, confirmed live
#   in pulse_lite.log 18:07:23-18:08:12 and 18:14:47-18:14:56 sessions).
# - Added a new CPU load signal read directly from /proc/stat (aggregate all-
#   core busy%, NOT per-policy/per-cluster -- see Open Questions/limitations
#   below). CPU_TIER now hystereses on this signal independently of GPU_BUSY.
# - GPU_TIER retains the exact v3.4 GPU_BUSY-driven hysteresis (HEAVY_UP/DOWN,
#   MEDIUM_UP/DOWN, HEAVY_DOWN_TICKS debounce), just renamed/scoped to GPU-only
#   caps (GPU_CAP node). Renamed MEDIUM_STUCK_TICKS -> GPU_MEDIUM_STUCK_TICKS
#   for clarity; kept as a safety net, but expected to fire less often now
#   that CPU is no longer artificially throttled by the GPU-driven medium tier
#   (untested hypothesis -- see Open Questions).
# - CPU_TIER gets its own mirrored hysteresis (CPU_HEAVY_UP/DOWN,
#   CPU_MEDIUM_UP/DOWN) and its own heavy->medium descent debounce
#   (CPU_HEAVY_DOWN_TICKS), structurally identical to GPU's but with
#   independent, currently UNVALIDATED PLACEHOLDER threshold values.
# - Thermal sub-tier logic (heavy_high/heavy_mid/heavy_low) is now also split:
#   CPU_HTIER reacts only to CTEMP (max of cpuss-* zones), GPU_HTIER reacts
#   only to GTEMP (max of gpuss-* zones). This is a behavior change from
#   v3.2-v3.4, where EITHER signal crossing its threshold downgraded BOTH
#   CPU and GPU together (worst-case-wins across domains). Now each domain
#   only downgrades on its own thermal signal. Rationale: matches the same
#   CPU/GPU independence principle as the tier split above; a hot GPU should
#   not force CPU clocks down if CPU itself is cool, and vice versa.
# - Force-tier testing hooks split accordingly: /sdcard/pulse_lite.force still
#   works as before (forces BOTH domains to the same tier, for backward-
#   compatible quick testing). Two new independent hooks added:
#   /sdcard/pulse_lite.force_cpu and /sdcard/pulse_lite.force_gpu, to force
#   each domain separately -- this is the primary tool for validating the
#   decoupling (e.g. force GPU=heavy while watching CPU stay in medium/idle
#   on its own signal).
# - KNOWN LIMITATION (flagged, not solved this version): CPU_BUSY is computed
#   from the aggregate "cpu " line in /proc/stat (all cores combined), not
#   per-policy/per-cluster. This is a coarse first cut per explicit scope
#   decision for this step ("krok 1 - cpu-load-based tier"). It answers
#   "is the CPU as a whole busy or not" but cannot yet answer "which cluster
#   needs the clocks" -- e.g. it cannot yet cap policy0 (A520) low while
#   keeping policy7 (X4) high if only the X4 core is loaded. That finer-grained
#   per-cluster throttling is explicitly the NEXT step, not this one.

# CHANGELOG v3.4 (vs v3.3):
# - Added HEAVY_DOWN_TICKS debounce: heavy->medium no longer triggers on a
#   single tick where GPU_BUSY dips below HEAVY_DOWN. Short busy% dips (e.g.
#   death animation, loading screen, cutscene) were causing an instant cap
#   drop mid-action, producing a visible frame hitch right as gameplay
#   resumed. Now requires HEAVY_DOWN_TICKS consecutive low-busy ticks before
#   actually dropping to medium. Any single tick back above HEAVY_DOWN resets
#   the counter (no partial credit).
# - Reduced MEDIUM_STUCK_TICKS 30 -> 8 (60s -> 16s).

# CHANGELOG v3.3 (vs v3.2):
# - Fixed feedback deadlock: if GPU is CPU-bound in medium tier, the daemon
#   would get stuck in medium forever. Added MEDIUM_STUCK_TICKS safety valve.

# CHANGELOG v3.2 (vs v3.0):
# - Dropped single TEMP_ZONE (skin-msm-therm proved to be ~16C off vs overlay
#   SKN -- vendor composite, not raw sysfs zone). Two independent raw signals
#   (CPU_TEMP, GPU_TEMP) now drive thermal sub-tier decisions.
...
# -- GPU hysteresis thresholds (%) -- unchanged from v3.4 ---------------------
HEAVY_UP=70
HEAVY_DOWN=45
MEDIUM_UP=20
MEDIUM_DOWN=10
...
# -- GPU medium deadlock safety valve (v3.3, renamed in v3.5) -----------------
# Kept as a fallback net. Expected to fire less often now that CPU_TIER no
# longer depends on GPU_TIER (untested hypothesis -- verify in soak test).
GPU_MEDIUM_STUCK_TICKS=8   # 8 * INTERVAL(2s) = 16s

# -- CPU hysteresis thresholds (%) -- NEW in v3.5, PLACEHOLDER/UNVALIDATED ----
# Mirrors GPU thresholds structurally. Driven by aggregate /proc/stat busy%,
# NOT per-policy. Needs a dedicated soak test (see handoff Proposed Scope).
CPU_HEAVY_UP=70
CPU_HEAVY_DOWN=45
CPU_MEDIUM_UP=20
CPU_MEDIUM_DOWN=10

# -- CPU heavy->medium descent debounce -- NEW in v3.5, PLACEHOLDER -----------
...
# v3.3 notes. CPU_HOT confirmed in right ballpark (cpuT=78300-78700 observed).
# CPU_COOL_HOT, GPU_HOT, GPU_COOL_HOT, both HOTTER values remain UNVALIDATED.
# v3.5 change: CTEMP now only affects CPU_HTIER, GTEMP now only affects
# GPU_HTIER (previously worst-case-wins across both domains -- see changelog).
CPU_HOT=78000          # cpuss max above this -> CPU heavy_mid
CPU_HOTTER=85000       # cpuss max above this -> CPU heavy_low
CPU_COOL_HOT=74000     # cpuss max below this -> CPU allowed back to heavy_high
CPU_COOL_HOTTER=81000  # cpuss max below this -> CPU allowed back to heavy_mid
...
# -- NEW in v3.5: aggregate CPU busy% from /proc/stat --------------------------
# Reads the "cpu " summary line (all cores combined). Computes delta-based
# busy% since the previous tick. Coarse signal -- see KNOWN LIMITATION above.
PREV_CPU_TOTAL=0
PREV_CPU_IDLE=0
CPU_STAT_INIT=0
...
# -- startup --------------------------------------------------------------------
rm -f "$SENTINEL"
log "START: pid=$$ ppid=$(cut -d' ' -f4 /proc/$$/stat 2>/dev/null) v3.5"
...
# prime the /proc/stat baseline before the loop so tick 1 has a real delta
read_cpu_busy >/dev/null
...
while true; do
  ...
  if [ "$CPU_TIER" != "$PREV_CPU_TIER" ] || [ "$CPU_HTIER" != "$PREV_CPU_HTIER" ] \
    ...
    log "cpu_tier=$CPU_TIER/$CPU_HTIER cpu_busy=${CPU_BUSY_VAL}% cpuT=${CTEMP} caps: p0=$C0 p2=$C2 p5=$C5 p7=$C7 | gpu_tier=$GPU_TIER/$GPU_HTIER gpu_busy=${GPU_BUSY_VAL}% gpuT=${GTEMP} gpu_cap=$GC"
