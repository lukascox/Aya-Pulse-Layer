#!/bin/sh
# pulse_lite v3.4 -- AutoTDP daemon, KONKR Pocket FIT / Snapdragon G3 Gen 3
...
# - Reduced MEDIUM_STUCK_TICKS 30 -> 8 (60s -> 16s). The old 60s safety valve
#   for CPU-bound medium-deadlock (see v3.3 notes) was far too slow in
#   practice -- a kid stuck at 15fps for a minute before relief is not
#   acceptable. 16s is short enough to feel responsive, long enough to avoid
#   escalating on a single brief loading-screen spike.
# - Both new counters reset on forced tier changes and on any tier resolved
#   via the normal BUSY thresholds (idle/heavy immediate paths untouched).
...
# Deploy: adb push pulse_lite_v3.4.sh /sdcard/pulse_lite.sh
...
# -- heavy->medium descent debounce (v3.4) ------------------------------------
# Do NOT drop out of heavy on a single low-busy tick (death animation, loading
# screen, cutscene all dip GPU_BUSY briefly). Require HEAVY_DOWN_TICKS
# consecutive ticks below HEAVY_DOWN before actually stepping down to medium.
# Any tick back above HEAVY_DOWN resets the counter to 0 (no partial credit).
HEAVY_DOWN_TICKS=5      # 5 * INTERVAL(2s) = 10s sustained low-busy required

# -- medium deadlock safety valve (v3.3, tuned in v3.4) -----------------------
...
# v3.3 had this at 30 ticks (60s) -- way too slow, kid-noticeable stutter.
MEDIUM_STUCK_TICKS=8    # 8 * INTERVAL(2s) = 16s
...
while true; do
  ...
  if [ -n "$FORCED" ]; then
    ...
    case $TIER in
      ...
      medium)
        ...
        ;;
      heavy)
        if [ "$BUSY" -lt "$HEAVY_DOWN" ]; then
          HEAVY_DOWN_TICK=$((HEAVY_DOWN_TICK + 1))
          if [ "$HEAVY_DOWN_TICK" -ge "$HEAVY_DOWN_TICKS" ]; then
            TIER=medium
            HEAVY_DOWN_TICK=0
            MEDIUM_TICK=0
          fi
  ...
  if [ "$TIER" != "$PREV_TIER" ] || [ "$HTIER" != "$PREV_HTIER" ] || [ $((TICK % 60)) -eq 0 ]; then
    ...
    if [ "$TIER" = "heavy" ]; then
      log "tier=$TIER/$HTIER gpu=${BUSY}% cpuT=${CTEMP} gpuT=${GTEMP} caps: p0=$C0 p2=$C2 p5=$C5 p7=$C7 heavy_down_tick=${HEAVY_DOWN_TICK}"
    else
